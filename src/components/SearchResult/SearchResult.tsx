...
const SearchResult = () => {
  ...
  useEffect(() => {
    const fetchSearchMovies = async () => {
      const res = await getSearchedMovies(
    ...
    <div className="search-result">
      <div className="search-result-container">
        <div className="search-result-sidebar">
          <div className="search-result-sidebar-header">Search Results</div>
          <ul className="search-results-groups">
            <li
              className={isActive === "movies_search" ? "isActive-search" : ""}
              id="movies_search"
              onClick={handleClick}
            >
              <h4>Movies</h4>
            ...
            <li
              id="tv_shows_search"
              className={
                isActive === "tv_shows_search" ? "isActive-search" : ""
              }
              onClick={handleClick}
            >
              <h4>TV shows</h4>
          ...
          {loopingMedia.length > 0 ? (
            <>
                ...
                <div className="search-result-main" key={movie.id}>
                  <div className="search-result-card">
                    ...
                    <div className="search-res-body">
                      ...
                      <div className="search-body-date">
                        {movie.name
                          ? formatDate(movie.first_air_date)
                          : formatDate(movie.release_date)}
                      </div>
                      <div className="search-body-overview">
                        {movie.overview.length > 256
                          ? movie.overview.slice(0, 256) + "..."
                          : movie.overview}{" "}
                      </div>
                    </div>
                  </div>
              ...
              {numMoviesRes && numTVRes ? (
                <ReactPaginate
                  ...
                  pageCount={
                    loopingMedia[0].name
                      ? numTVRes?.total_pages
                      : numMoviesRes?.total_pages
                  }
            ...
            <>
              ...
              <div className="not-found-text">
                No Media with this title found,
                <Link to="/">Search Movies or TV shows Again</Link>
              </div>
            </>
          ) : (
            <Spinner />
